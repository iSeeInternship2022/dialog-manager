...
from Buisiness.BT.structure.nodes.composite.SequenceNode import SequenceNode
...
def giveNodesProperties(parsed_tree, nodes):
	for n in nodes:
		...
		current_node.prop = parsed_tree.BT_nodes[n]["properties"]
		print(current_node.toString() + " set")

		#====== COMPOSITE ======
		if(type(current_node) is SequenceNode
		or type(current_node) is PriorityNode):
			makeCompositeNode(current_node, parsed_tree, nodes)
			


		#====== DECORATOR ======
		elif(type(current_node) is RepTillFailNode
		or type(current_node) is RepTillSuccNode
		or type(current_node) is InverterNode
		or type(current_node) is LimitActivationNode):
			makeSimpleDecorator(current_node, parsed_tree, nodes)


		# #====== CONDITION ======
		# elif(type(current_node) is EqualNode):
		# 	makeEqualNode(current_node, parsed_tree)

		# elif(type(current_node) is ConditionNode):
		# 	makeConditionNode(current_node, parsed_tree)


		# #====== ACTION ======
		# elif(type(current_node) is ExplanationExperienceNode):
		# 	makeExplanationExperienceNode(current_node, parsed_tree)

		# elif(type(current_node) is InformationNode):
		# 	makeInformationNode(current_node, parsed_tree)

		# elif(type(current_node) is QuestionNode):
		# 	makeQuestionNode(current_node, parsed_tree)

		# elif(type(current_node) is WorldModifierNode):
		# 	makeWorldModifierNode(current_node, parsed_tree)
